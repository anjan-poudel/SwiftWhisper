...
struct ggml_metal_context * ggml_metal_init(int n_cb) {
    fprintf(stderr, "[ggml-metal] init: allocating\n");
    ...
    // load library
    {
        NSBundle * bundle = nil;
        NSError * error = nil;
        // Search every bundle (main app + SPM resource bundles) for the
    // precompiled library — SPM .copy resources don't land in mainBundle.
    NSString * libPath = nil;
    for (NSBundle * b in [NSBundle allBundles]) {
        libPath = [b pathForResource:@"default" ofType:@"metallib"];
        if (libPath) break;
    }
    if (!libPath) {
        // SPM resource bundles live inside the main bundle as
        // <Package>_<Target>.bundle and may not be in allBundles yet.
        NSBundle * mb = [NSBundle mainBundle];
        NSArray * names = @[@"SwiftWhisper_whisper_cpp", @"SwiftWhisper_SwiftWhisper"];
        for (NSString * n in names) {
            NSBundle * rb = [NSBundle bundleWithPath:[mb pathForResource:n ofType:@"bundle"]];
            libPath = [rb pathForResource:@"default" ofType:@"metallib"];
            if (libPath) break;
        }
    }
        if (libPath != nil) {
            ...
            if (ggmlMetalPathResources) {
                ...
                for (NSBundle * b in [NSBundle allBundles]) {
                    sourcePath = [b pathForResource:@"ggml-metal" ofType:@"metal"];
                    if (sourcePath) break;
                }
            }
            if (sourcePath == nil) {
...
void ggml_metal_graph_compute(
    ...
    fprintf(stderr, "[ggml-metal] graph compute: n_nodes = %d, concur = %s\n",
            gf->n_nodes, has_concur ? "yes" : "no");
    ...
    for (int cb_idx = 0; cb_idx < n_cb; ++cb_idx) {
        ...
        dispatch_async(ctx->d_queue, ^{
            ...
            for (int ind = node_start; ind < node_end; ++ind) {
                ...
                fprintf(stderr, "[ggml-metal] encoding node %d/%d: %s\n",
                        ind, gf->n_nodes,
                        i >= 0 ? ggml_op_name(gf->nodes[i]->op) : "skip");
